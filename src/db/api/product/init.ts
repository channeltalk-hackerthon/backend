...
const createCollection = () => {
    return createProduct("dummy", "dummy", "KRW", 10000);
}

const dropProductCollection = () => {
    const conn = mongoose.createConnection(`${process.env.MONGO_URL}`)
    return new Promise((res, rej) => {
        conn.dropCollection('products')
        .then(() => {
        ...
        createCollection()
            ...
            .then(() => {
                const saveMockPromises = mockProduct.map((product) => {
                    return createProduct(product.product_name, product.product_description, product.currency, product.price)
                })
